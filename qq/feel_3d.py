...
if __name__ == "__main__":
    ...
    #             z_.append(p[0][2])
    #     plt.plot(x, y, z,'bo', x_, y_,z_, 'rx')  # 在图里yi=1的点用点表示，yi=-1的点用叉表示
    #     plt.axis([-1, 2, -1, 2, -1, 2])  # 横纵坐标上下限
    #     plt.grid(True)  # 显示网格
    #     plt.xlabel('x1')  # 这里我修改了原文表示
    #     plt.ylabel('x2')  # 为了和原理中表达方式一致，横纵坐标应该是x1,x2
    #     plt.zlabel('x3')  # 为了和原理中表达方式一致，横纵坐标应该是x1,x2
    ...
    #     # x1 = 0
    #     # y1 = -(b + w[0] * x1) / w[1]
    #     w1, w2, b = [1.3, -1.3, -1.0999999999999999]
    #     X = np.arange(0, 1, 0.1)
    #     Y = np.arange(0, 1, 0.1)
    #     X, Y = np.meshgrid(X, Y)
    #     Z = w1 * X + w2 * Y + b
